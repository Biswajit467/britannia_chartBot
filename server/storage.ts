...
export interface IStorage {
  ...
  createTroubleshootingIssue(
    issue: InsertTroubleshootingIssue
  ): Promise<TroubleshootingIssue>;
  createSupportInteraction(
    interaction: InsertSupportInteraction
  ): Promise<SupportInteraction>;
  getSupportInteractionsByType(
    issueType: string
  ): Promise<SupportInteraction[]>;
  getAllMaintenanceSchedules(): Promise<MaintenanceSchedule[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getTroubleshootingIssuesByType(
    type: string
  ): Promise<TroubleshootingIssue[]> {
    return await db
      .select()
      .from(troubleshootingIssues)
      .where(eq(troubleshootingIssues.type, type));
  }

  async searchTroubleshootingIssues(
    query: string
  ): Promise<TroubleshootingIssue[]> {
    return await db
      .select()
  ...
  async createTroubleshootingIssue(
    issue: InsertTroubleshootingIssue
  ): Promise<TroubleshootingIssue> {
    ...
    const [createdIssue] = await db
      .select()
      .from(troubleshootingIssues)
      .where(eq(troubleshootingIssues.title, issue.title));
    return createdIssue;
  }

  async createSupportInteraction(
    interaction: InsertSupportInteraction
  ): Promise<SupportInteraction> {
    await db.insert(supportInteractions).values(interaction);
    const [createdInteraction] = await db
      .select()
      .from(supportInteractions)
      .orderBy(desc(supportInteractions.id))
      .limit(1);
    return createdInteraction;
  }

  async getSupportInteractionsByType(
    issueType: string
  ): Promise<SupportInteraction[]> {
    return await db
      .select()
      .from(supportInteractions)
      .where(eq(supportInteractions.issueType, issueType));
  }
  ...
  async getMaintenanceSchedulesByFrequency(
    frequency: string
  ): Promise<MaintenanceSchedule[]> {
    return await db
      .select()
      .from(maintenanceSchedules)
      .where(eq(maintenanceSchedules.frequency, frequency));
  }
  ...
  async createMaintenanceSchedule(
    ...
    const [createdSchedule] = await db
      .select()
      .from(maintenanceSchedules)
      .where(eq(maintenanceSchedules.title, schedule.title));
    return createdSchedule;
  }
